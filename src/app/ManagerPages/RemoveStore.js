...
export default function RemoveStoreItem() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      ...
      if (!po_id) {
        setError("Missing post office ID")
        setLoading(false)
        return
      }
  ...
  const openConfirmDialog = () => {
    if (!selectedItem) {
      setMessage({ type: "error", text: "⚠ You must select an item to remove." })
      return
    }
  ...
  const handleRemoveItem = async () => {
    ...
    try {
      ...
      const removedItemName = items.find((item) => item.item_id === selectedItem)?.item_name || "Item"
    ...
    <Container maxWidth="md" sx={{ my: 4 }}>
          ...
          <Box sx={{ position: "relative", zIndex: 1, display: "flex", alignItems: "center" }}>
            ...
            <Box>
              ...
              <Typography variant="subtitle1" sx={{ mt: 1, opacity: 0.9 }}>
                Make an item unavailable to users moving forward
              </Typography>
            </Box>
          </Box>
        </Box>
      ...
      {!loading && !error && items.length > 0 && (
        <Box sx={{ display: "flex", gap: 2, mb: 4, flexWrap: "wrap" }}>
          ...
          >
            <InventoryIcon sx={{ color: "#D32F2F", mr: 1.5, fontSize: 28 }} />
            <Box>
          ...
          >
            <StorefrontIcon sx={{ color: "#D32F2F", mr: 1.5, fontSize: 28 }} />
            <Box>
              <Typography variant="body2" color="text.secondary">
                Categories
              </Typography>
              <Typography variant="h5" fontWeight="bold" color="#D32F2F">
                {[...new Set(items.map((item) => item.item_category))].length}
              </Typography>
            </Box>
          </Paper>
          ...
          >
            <AttachMoneyIcon sx={{ color: "#D32F2F", mr: 1.5, fontSize: 28 }} />
            <Box>
              <Typography variant="body2" color="text.secondary">
                Avg. Price
              </Typography>
      ...
      >
        <Box sx={{ mb: 3 }}>
          <Typography variant="h5" fontWeight="bold" color="#D32F2F" gutterBottom>
            Select an item to remove
          </Typography>
          <Typography variant="body1" color="text.secondary">
            Any unfinished orders placed on this item won't be cancelled, but future orders won't be allowed.
          </Typography>
        </Box>
        ...
        {loading ? (
          ...
          <>
                ...
                {items.map((item) => (
                  <MenuItem key={item.item_id} value={item.item_id}>
                    <Box sx={{ display: "flex", justifyContent: "space-between", width: "100%", alignItems: "center" }}>
                      <Typography fontWeight={item.stock <= 5 ? "bold" : "normal"}>{item.item_name}</Typography>
                      <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                        <Chip label={item.item_category} size="small" sx={{ fontSize: "0.75rem" }} />
                        <Chip
                          label={`Stock: ${item.stock}`}
                          size="small"
                          color={item.stock <= 5 ? "error" : item.stock <= 15 ? "warning" : "default"}
                          sx={{ fontSize: "0.75rem" }}
                        />
            ...
            <Box sx={{ display: "flex", alignItems: "center", mb: 3 }}>
              ...
              <Typography variant="subtitle1" fontWeight="medium" color="#D32F2F">
      ...
      >
        <DialogTitle id="alert-dialog-title" sx={{ bgcolor: "#FFEBEE", color: "#B71C1C" }}>
        ...
        <DialogContent sx={{ mt: 2 }}>
          <DialogContentText id="alert-dialog-description">
            Are you sure you want to remove <strong>{selectedItemDetails?.item_name}</strong> from the store inventory?
