...
export default function ReStock() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const response = await fetch(`https://final-po-api.vercel.app/api/ViewStore?po_id=${user.po_id}`)
  ...
  const handleUpdateStock = async () => {
    ...
    try {
      const response = await fetch("https://final-po-api.vercel.app/api/UpdateStock", {
        method: "POST",
