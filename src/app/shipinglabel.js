  ...
  useEffect(() => {
    if (payload && payload.customers_id) {
      const fetchTrackingNumber = async () => {
        try {
          const response = await fetch("https://final-po-api.vercel.app/api/getCustomerPackages", {
            method: "POST",
    ...
    if (payload && payload.po_id) {
      const fetchPostOfficeData = async () => {
        try {
          ...
          const response = await fetch("https://final-po-api.vercel.app/api/getPostOfficeLocations")
