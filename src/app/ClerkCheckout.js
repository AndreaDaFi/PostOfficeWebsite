...
export default function PackageCheckout() {
  ...
  useEffect(() => {
    const fetchTaxRate = async () => {
      try {
        ...
        const response = await fetch(`https://final-po-api.vercel.app/api/GetTax?address_id=${payload.origin_address_id}`)
  ...
  const handleCheckout = async (e) => {
    ...
    const { origin_address_id, ...checkoutData } = { ...payload };
    console.log("data im passing to the api: ", checkoutData);


    try {
      const response = await fetch("https://final-po-api.vercel.app/api/ClerkPackageCheckout", {
        method: "POST",
        headers: {
