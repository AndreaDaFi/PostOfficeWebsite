...
export default function AddPackage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!packageData.customerID || !packageData.customerFirstName || !packageData.customerLastName || !packageData.receiverFirstName || !packageData.receiverLastName || !packageData.customerStreet || !packageData.receiverStreet) {
      setError("⚠ Please fill in all required fields.");
      return;
    }

    try {
      const response = await fetch("https://your-api-url.com/add-package", {
      ...
      setSuccessMessage("🎉 Package successfully added!");
    } catch (err) {
  ...
  return (
    <Container maxWidth="md" style={{ marginTop: "20px", marginBottom: "20px" }}>
      <Paper elevation={3} style={{ padding: "20px", borderRadius: "12px", textAlign: "center" }}>
        ...
        <Grid container spacing={2}>
          <Grid item xs={4}><TextField fullWidth label="First Name" name="receiverFirstName" variant="outlined" value={packageData.receiverFirstName} onChange={handleChange} /></Grid>
          <Grid item xs={4}><TextField fullWidth label="Last Name" name="receiverLastName" variant="outlined" value={packageData.receiverLastName} onChange={handleChange} /></Grid>
