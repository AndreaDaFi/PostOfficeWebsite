...
export default function CustomerPackageEntry() {
  ...
  const [packageData, setPackageData] = useState({
    ...
    fastdelivery: false,
    po_id: user.po_id,
    empAdd: user.employees_address_id
  });
  ...
  const handleSubmit = async () => {
    ...
    if (
      ...
      !validateEmail(packageData.customerEmail) ||
    ...
    const payload = {
      ...
      origin_address_id: packageData.empAdd
    };
    console.log("data being passed", payload);

    navigate("/ClerkCheckout", { state: { totalPrice, payload } });
  };
    ...
    <Container maxWidth="md" sx={{ my: 4 }}>
      ...
      >
        ...
        <Typography variant="body1" sx={{ mb: 2 }}>
          A customer must have an account created in order for us to deliver their package. Please enter the email associated with this customer's account.
        </Typography>
        <Grid item xs={8}>
          <TextField
            fullWidth
            ...
            onChange={(e) =>
              setPackageData({ ...packageData, customerEmail: e.target.value })
            }
            helperText="Max 45 characters"
            error={packageData.customerEmail.length > 45}
          />
        </Grid>
